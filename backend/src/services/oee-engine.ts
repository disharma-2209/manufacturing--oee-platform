...
export function calculateOEE(
  ...
  const byEquipment: EquipmentMetrics[] = [];
...
function computeLineMetrics(
  ...
  const qualityDefects = incidents.filter((i) =>
    i.cause_category?.toLowerCase().includes('qly') ||
    i.cause_category?.toLowerCase().includes('quality')
  ).length;
...
function markChronicFailures(equipMetrics: EquipmentMetrics[], incidents: RawIncident[]): void {
...
function detectAnomalies(byLine: LineMetrics[], byEquipment: EquipmentMetrics[]): AnomalyAlert[] {
...
export function buildParetoData(incidents: RawIncident[]): {
  byCategory: { category: string; hours: number; count: number; cumulative: number }[];
  ...
  const causeMap = new Map<string, { category: string; hours: number; count: number }>();
  // station key: "station||line"
  const stationMap = new Map<string, { line: string; hours: number; count: number; catCounts: Map<string, number> }>();
  ...
  for (const inc of incidents) {
    ...
    const existingSt = stationMap.get(stKey) || { line, hours: 0, count: 0, catCounts: new Map<string, number>() };
...
export function detectCascadeAndRepeatFailures(incidents: RawIncident[]): RawIncident[] {
  ...
  const equipMap = new Map<string, RawIncident[]>();
