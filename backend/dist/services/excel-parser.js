...
exports.deriveRootCauseCategory = deriveRootCauseCategory;
...
function deriveShift(reportTime) {
    if (!reportTime)
        return 'Unknown';
...
function getWeekNumber(dateStr) {
    if (!dateStr)
        return { week: 0, year: new Date().getFullYear() };
...
function deriveRootCauseCategory(causeCategory, cause, remarks, repairDescription) {
...
function getCol(row, ...keys) {
    ...
    for (const k of Object.keys(row))
        rowLower[k.toLowerCase().trim().replace(/\s+/g, ' ')] = row[k];
...
function parseExcelFile(filePath) {
    ...
    const rawSheetName = sheets.find((s) => s.toLowerCase().includes('raw') || s.toLowerCase().includes('data') || s.toLowerCase().includes('incident')) || sheets[0];
    const rawSheet = workbook.Sheets[rawSheetName];
    const rawData = XLSX.utils.sheet_to_json(rawSheet, { defval: null });
    if (rawData.length === 0) {
    ...
    for (const row of rawData) {
        const lineRaw = String(getCol(row, 'line', 'LINE', 'production line', 'prod line', 'line no', 'line_no') || '').trim();
        const line = LINE_MAP[lineRaw] || lineRaw || 'Unknown';
        const reportTime = parseExcelDate(getCol(row, 'report time', 'REPORT TIME', 'report_time', 'reporttime', 'reported time', 'date reported', 'date/time', 'datetime', 'timestamp', 'date'));
        const dtAckTime = parseExcelDate(getCol(row, 'dt ack time', 'DT ACK TIME', 'ack time', 'acknowledged time', 'dt_ack_time'));
        const completedTime = parseExcelDate(getCol(row, 'completed time', 'COMPLETED TIME', 'completed_time', 'completion time', 'date completed'));
        const endTime = parseExcelDate(getCol(row, 'end time', 'END TIME', 'end_time', 'endtime', 'resolved time'));
        const durationHours = timeStringToHours(getCol(row, 'duration', 'DURATION', 'duration (hrs)', 'duration(hrs)', 'downtime duration', 'downtime hours', 'dt duration', 'hours'));
        const responseHours = timeStringToHours(getCol(row, 'response time', 'RESPONSE TIME', 'response_time', 'response time (hrs)'));
        const resolutionHours = timeStringToHours(getCol(row, 'resolution time', 'RESOLUTION TIME', 'resolution_time', 'resolution time (hrs)'));
        const { week, year } = getWeekNumber(reportTime);
        const shift = String(getCol(row, 'shift', 'SHIFT', 'shift name') || deriveShift(reportTime)).trim() || deriveShift(reportTime);
        ...
        const rawCauseCategory = String(getCol(row, 'cause category', 'CAUSE CATEGORY', 'category', 'failure category', 'fault category', 'downtime category', 'dt category', 'root cause category') || '').trim();
        const rawCause = String(getCol(row, 'cause', 'CAUSE', 'root cause', 'failure cause', 'fault', 'reason', 'description', 'failure description') || '').trim();
        ...
        const equipment = String(getCol(row, 'equipment', 'EQUIPMENT', 'machine', 'asset', 'equipment name', 'machine name', 'asset name') || '').trim();
        // Station: explicit column first, then fall back to equipment name (equipment IS the station in SMT lines)
        const station = String(getCol(row, 'station', 'STATION', 'station name', 'work station', 'workstation', 'work center', 'workcenter', 'wc', 'process station', 'sub station') || equipment).trim();
        incidents.push({
            line,
            equipment,
            status: String(getCol(row, 'status', 'STATUS', 'incident status', 'state') || 'CLOSED').trim(),
            cause_category: derived.category,
            cause: derived.cause,
            product_code: String(getCol(row, 'product code', 'PRODUCT CODE', 'product', 'part number', 'part no', 'sku', 'model') || '').trim(),
            team: String(getCol(row, 'team', 'TEAM', 'responsible team', 'maintenance team', 'dept', 'department') || '').trim(),
            remarks: String(getCol(row, 'remarks', 'REMARKS', 'comments', 'notes', 'remark') || '').trim(),
            repair_description: String(getCol(row, 'repair description', 'REPAIR DESCRIPTION', 'repair', 'action taken', 'corrective action', 'fix description') || '').trim(),
            report_time: reportTime || '',
            report_by: String(getCol(row, 'report by', 'REPORT BY', 'reported by', 'reporter', 'operator', 'raised by') || '').trim(),
            dt_ack_time: dtAckTime || '',
            dt_ack_by: String(getCol(row, 'dt ack by', 'DT ACK BY', 'acknowledged by', 'ack by') || '').trim(),
            completed_time: completedTime || '',
            complete_by: String(getCol(row, 'complete by', 'COMPLETE BY', 'completed by', 'resolved by', 'closed by') || '').trim(),
            end_time: endTime || '',
    ...
    const dominantWeek = weekNumbers.length > 0
        ? weekNumbers.sort((a, b) => weekNumbers.filter(v => v === b).length - weekNumbers.filter(v => v === a).length)[0]
        : 0;
